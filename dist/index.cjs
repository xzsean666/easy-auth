...
var MemoryStorageAdapter = class {
  ...
  async createUser(data) {
    ...
    const id = data.id || `usr_${crypto.randomUUID().replace(/-/g, "").slice(0, 16)}`;
  ...
  async createIdentity(data) {
    ...
    const id = data.id || `idn_${crypto.randomUUID().replace(/-/g, "").slice(0, 16)}`;
    const now = /* @__PURE__ */ new Date();
    const identity = {
      id,
      userId: data.userId,
...
function getDatabaseSyncClass() {
  try {
    const req = typeof __require !== "undefined" ? __require : module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)));
...
var JwtService = class {
  ...
  async verify(token) {
    ...
    try {
      ...
      if (err instanceof jose.errors.JWTExpired || err?.code === "ERR_JWT_EXPIRED") {
...
var Web3Provider = class {
  ...
  async verifyAndExtract(credentials) {
    ...
    if (!address || typeof address !== "string" || !viem.isAddress(address.toLowerCase())) {
    ...
    try {
      if (this.client && typeof this.client.verifyMessage === "function") {
        ...
        isValid = await viem.verifyMessage({
          address: normalizedAddress,
          message,
          signature
        });
...
var GoogleProvider = class _GoogleProvider {
  ...
  static googleJwks = jose.createRemoteJWKSet(
  ...
  async verifyIdToken(idToken) {
    ...
    if (mode === "jwks") {
      try {
        ...
        const { payload } = await jose.jwtVerify(idToken, _GoogleProvider.googleJwks, {
...
exports.EasyAuthService = class EasyAuthService {
  constructor(options) {
    this.options = options;
...
exports.EasyAuthService = __decorateClass([
  common.Injectable(),
  __decorateParam(0, common.Inject(EASY_AUTH_OPTIONS))
], exports.EasyAuthService);
exports.EasyAuthGuard = class EasyAuthGuard {
  ...
  async canActivate(context) {
    ...
    if (!authHeader || typeof authHeader !== "string" || !authHeader.startsWith("Bearer ")) {
      throw new common.UnauthorizedException("Missing or malformed Authorization header with Bearer token");
    }
    const token = authHeader.slice(7).trim();
    if (!token) {
      throw new common.UnauthorizedException("Empty Bearer token provided");
    }
    try {
      ...
      throw new common.UnauthorizedException(err.message || "Invalid or expired authentication token");
    }
  }
};
exports.EasyAuthGuard = __decorateClass([
  common.Injectable(),
  __decorateParam(0, common.Inject(exports.EasyAuthService)),
  __decorateParam(1, common.Optional()),
  __decorateParam(1, common.Inject(core.Reflector))
], exports.EasyAuthGuard);
...
exports.EasyAuthController = class EasyAuthController {
  ...
  executeOAuthRedirect(provider, redirect, req, res) {
    ...
    try {
      ...
      return { statusCode: common.HttpStatus.FOUND, url: authUrl };
    } catch (err) {
      this.handleError(err);
    }
  }
  async handleOAuthCallback(provider, code, state, error, errorDescription, req, res) {
    ...
    if (error || !code) {
      ...
      return { statusCode: common.HttpStatus.FOUND, url: errorUrl, error: errorMsg };
    }
    try {
      const defaultCallbackUri = `${protocol}://${host}/api/auth/callback/${provider}`;
      const envCallback = process.env[`${provider.toUpperCase()}_REDIRECT_URI`] || process.env.AUTH_CALLBACK_URL;
      ...
      return {
        statusCode: common.HttpStatus.FOUND,
        url: successUrl,
        token: result.token,
      ...
      return { statusCode: common.HttpStatus.FOUND, url: errorUrl, error: err.message };
    }
  }
};
__decorateClass([
  Public(),
  common.Get("nonce"),
  __decorateParam(0, common.Query("address"))
], exports.EasyAuthController.prototype, "getNonce", 1);
__decorateClass([
  Public(),
  common.Post("login"),
  __decorateParam(0, common.Body())
], exports.EasyAuthController.prototype, "login", 1);
__decorateClass([
  common.Post("link"),
  __decorateParam(0, common.Body()),
  __decorateParam(1, CurrentUser("id"))
], exports.EasyAuthController.prototype, "link", 1);
__decorateClass([
  common.Post("unlink"),
  __decorateParam(0, common.Body()),
  __decorateParam(1, CurrentUser("id"))
], exports.EasyAuthController.prototype, "unlink", 1);
__decorateClass([
  common.Get("me"),
  __decorateParam(0, common.Headers("authorization")),
  __decorateParam(1, CurrentUser())
], exports.EasyAuthController.prototype, "me", 1);
__decorateClass([
  common.Get("identities"),
  __decorateParam(0, common.Query("userId")),
  __decorateParam(1, CurrentUser("id"))
], exports.EasyAuthController.prototype, "listIdentities", 1);
__decorateClass([
  Public(),
  common.Get("oauth/:provider"),
...
__decorateClass([
  ...
  __decorateParam(0, common.Param("provider")),
  __decorateParam(1, common.Query("code")),
  __decorateParam(2, common.Query("state")),
  __decorateParam(3, common.Query("error")),
  __decorateParam(4, common.Query("error_description")),
  __decorateParam(5, common.Req()),
  __decorateParam(6, common.Res())
], exports.EasyAuthController.prototype, "handleOAuthCallback", 1);
exports.EasyAuthController = __decorateClass([
  common.Controller("api/auth"),
  common.UseGuards(exports.EasyAuthGuard),
  __decorateParam(0, common.Inject(exports.EasyAuthService))
], exports.EasyAuthController);

// src/nestjs/easy-auth.module.ts
exports.EasyAuthModule = class EasyAuthModule {
  /**
  ...
  static forRoot(options) {
    if (options.routePrefix) {
      ...
      Reflect.defineMetadata("path", cleanPrefix, exports.EasyAuthController);
    }
    const controllers = options.disableController ? [] : [exports.EasyAuthController];
    const providers = [
      {
    ...
    if (options.globalGuard) {
      providers.push({
        ...
        useClass: exports.EasyAuthGuard
      });
    }
    return {
      module: exports.EasyAuthModule,
      controllers,
      providers,
      exports: [exports.EasyAuthService, exports.EasyAuthGuard]
    };
  }
  /**
  ...
  static forRootAsync(asyncOptions) {
    ...
    return {
      ...
      controllers: [exports.EasyAuthController],
      providers,
      exports: [exports.EasyAuthService, exports.EasyAuthGuard]
    };
  }
};
exports.EasyAuthModule = __decorateClass([
  common.Global(),
  common.Module({})
], exports.EasyAuthModule);
...
exports.EasyAuthError = EasyAuthError;
exports.GoogleOtpProvider = TotpProvider;
